...
export class RoleGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if(cachedPermissionId){
      ...
      currentPermissionId = await this.permissionService.getPermissionIdByPathAndMethod(path, method);
      await this.redisClient.set(permissionCacheKey, currentPermissionId.toString(), 'EX', 3600) //1hour
    }
    ...
    if(cached){
      ...
      await this.redisClient.set(roleCacheKey, JSON.stringify(permissionIds), 'EX', 3600)
    }
