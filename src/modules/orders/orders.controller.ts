...
export class OrdersController {
  ...
  @Post()
  @UseGuards(AuthGuard)
  async create(@Req() req: Request,@Body() createOrderDto: CreateOrderDto): Promise<APIResponseDTO<string>> {
  ...
  @Get(':id')
  async findOneOrder(@Param('id') id: string): Promise<APIResponseDTO<OrderResponseDTO>> {
  ...
  async changeStatus(@Param('id') id: string): Promise<APIResponseDTO<string>>{
    const result = await this.ordersService.changeStatus(Status.orderAccept,+id)
    return new APIResponseDTO<string>(true,200,result)
  }
}
