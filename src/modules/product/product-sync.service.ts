...
export class ProductSyncService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    const productDocs = products.map((p) => {
      ...
      return plainToInstance(ProductResponse, {
        ...p,
        priceAfterDis,
    ...
    for (const doc of productDocs) {
      try {
        ...
        Logger.error(`[Sync] Failed to index product ${doc.id}: ${err.message}`);
      }
    }

    Logger.log(`[Sync] Synced ${productDocs.length} products to Elasticsearch.`);
  }
}